...
def solveit(filename):
    ...
    img, pts = extractsudoku(filename)
    ...
    if(solved!=0):
        ...
        for i in range(9):
            for j in range(9):
                if(unsolved[i][j]==0):
                    cv.putText(blank, str(sudoku[i][j]), (j*40 + 10, i*40 + 30), cv.FONT_HERSHEY_COMPLEX, 1, (255, 0, 255), 2)
        cv.imshow('original', img)
        cv.imwrite('original.jpg', img)
        cv.imshow('solved',blank)
        cv.imwrite('solved.jpg', blank)
        M = cv.getPerspectiveTransform(pts[0], pts[1])
        original = cv.imread("filename")
        img = cv.warpPerspective(blank,M,(original.shape[1],original.shape[0]))
